...
func TestCollectionFailureStateSurvivesRestart(t *testing.T) {
	...
	if err := s1.RecordCollectionResult("soju", errors.New("parser changed"), now); err != nil {
		t.Fatal(err)
	}
	if err := s1.RecordCollectionResult("soju", errors.New("parser changed again"), now.Add(time.Minute)); err != nil {
		t.Fatal(err)
	}
	...
	if err := s2.RecordCollectionResult("soju", nil, now.Add(2*time.Minute)); err != nil {
		t.Fatal(err)
	}
	st, err = s2.CollectionFailureStats("soju")
	if err != nil {
...
func TestCollectorHealthRestoresPersistedLastError(t *testing.T) {
	...
	if err := s1.Save(PackageStat{Package: "soju", Downloads: 1, UpdatedAt: now}); err != nil {
		t.Fatal(err)
	}
	if err := s1.RecordCollectionResult("soju", errors.New("github markup changed"), now); err != nil {
		t.Fatal(err)
	}
	if err := s1.Close(); err != nil {
		t.Fatal(err)
