...
func testM1App(t *testing.T) *App {
	...
	pkgs := []string{"soju", "mineflayer"}
	return &App{
		cfg:           Config{Owner: "Ploos-AS", Packages: pkgs, PackagesExplicit: true},
		store:         s,
		collector:     GitHubHTMLCollector{},
...
func TestPackageSummary(t *testing.T) {
	...
	if s.Downloads != 150 || s.Downloads7d != 30 || s.Downloads30d != 50 {
		t.Fatalf("%+v", s)
	}
}
...
func TestOrgSummary(t *testing.T) {
	a := testM1App(t)
	s := a.orgSummary()
	if s.Downloads != 380 || s.Downloads7d != 50 || s.Downloads30d != 80 {
		t.Fatalf("%+v", s)
	}
}
...
func TestBadgeAndShieldsEndpoints(t *testing.T) {
	...
	a.handleM1Badge(w, r)
	if w.Code != 200 || !strings.Contains(w.Body.String(), "80") {
	...
	w = httptest.NewRecorder()
	a.handleShields(w, r)
	if w.Code != 200 || !strings.Contains(w.Body.String(), `"schemaVersion":1`) || !strings.Contains(w.Body.String(), `"message":"150"`) {
		t.Fatalf("%d %s", w.Code, w.Body.String())
	}
}

func TestPackageListEndpoint(t *testing.T) {
	a := testM1App(t)
	w := httptest.NewRecorder()
	r := httptest.NewRequest("GET", "/api/v1/packages", nil)
	a.handlePackageList(w, r)
	body := w.Body.String()
	if w.Code != 200 || !strings.Contains(body, `"source":"explicit"`) || !strings.Contains(body, `"mineflayer"`) || !strings.Contains(body, `"soju"`) {
...
func TestM1Metrics(t *testing.T) {
	...
	a.handleMetrics(w, r)
	body := w.Body.String()
	for _, want := range []string{"ghcr_downloads_7d", "ghcr_downloads_30d", "ghcr_org_downloads_total", "ghcr_org_downloads_30d", "ghcr_stats_packages", "ghcr_stats_discovery_up"} {
