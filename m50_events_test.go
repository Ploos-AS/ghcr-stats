...
func newM50TestApp(t *testing.T) *App {
	...
	if err := a.initM50(); err != nil { t.Fatal(err) }
	return a
}
...
func TestM50EventRoundTripAndFilters(t *testing.T) {
	...
	if err != nil || id == 0 { t.Fatalf("save id=%d err=%v",id,err) }
	items, err := a.store.Events("soju","collector_failed","error",10)
	if err != nil { t.Fatal(err) }
	if len(items)!=1 || items[0].Message!="boom" || !items[0].CreatedAt.Equal(when) { t.Fatalf("unexpected events: %#v",items) }
}
...
func TestM50EventsAPI(t *testing.T) {
	...
	var body struct { Owner string `json:"owner"`; Events []Event `json:"events"` }
	if err:=json.Unmarshal(w.Body.Bytes(),&body); err!=nil { t.Fatal(err) }
	if body.Owner!="Ploos-AS" || len(body.Events)!=1 || body.Events[0].Owner!="Ploos-AS" { t.Fatalf("unexpected body: %#v",body) }
}

func TestM50EventsAPIValidation(t *testing.T) {
	a := newM50TestApp(t)
	for _, u := range []string{"/api/v1/events?limit=0","/api/v1/events?severity=critical"} {
		w:=httptest.NewRecorder(); a.handleEvents(w,httptest.NewRequest(http.MethodGet,u,nil)); if w.Code!=http.StatusBadRequest { t.Fatalf("%s status=%d",u,w.Code) }
	}
	w:=httptest.NewRecorder(); a.handleEvents(w,httptest.NewRequest(http.MethodPost,"/api/v1/events",nil)); if w.Code!=http.StatusMethodNotAllowed { t.Fatalf("post status=%d",w.Code) }
}
