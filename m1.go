...
func metricValue(s MetricSummary, metric string) (int64, string, bool) {
	switch metric {
	case "pulls":
		return s.Downloads, "GHCR pulls", true
	case "pulls-7d":
		return s.Downloads7d, "GHCR pulls 7d", true
	case "pulls-30d":
		return s.Downloads30d, "GHCR pulls 30d", true
	default:
		return 0, "", false
	}
}
...
func (a *App) handleM1Badge(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if parts[0] == "org" {
		summary = a.orgSummary()
	} else {
		summary, err = a.packageSummary(parts[0])
	}
	if err != nil {
		http.NotFound(w, r)
		return
...
func (a *App) handleOrgJSON(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]any{
		...
		"downloads_7d":    s.Downloads7d,
		"downloads_30d":   s.Downloads30d,
