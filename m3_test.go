...
func seedM3(t *testing.T, a *App, now time.Time) {
	...
	for _, st := range []PackageStat{
		...
		if err := a.store.Save(st); err != nil { t.Fatal(err) }
	}
}
...
func TestM3HistoryAPIAndDashboard(t *testing.T) {
	...
	if rr.Code != http.StatusOK { t.Fatalf("history status=%d body=%s", rr.Code, rr.Body.String()) }
	var payload struct { Package string `json:"package"`; Period string `json:"period"`; Points []HistoryPoint `json:"points"` }
	if err := json.Unmarshal(rr.Body.Bytes(), &payload); err != nil { t.Fatal(err) }
	if payload.Package != "alpha" || payload.Period != "90d" || len(payload.Points) != 3 { t.Fatalf("payload=%#v", payload) }

	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/api/v1/rankings?period=30d", nil))
	if rr.Code != http.StatusOK || !strings.Contains(rr.Body.String(), `"package":"alpha"`) { t.Fatalf("ranking: %d %s", rr.Code, rr.Body.String()) }

	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/", nil))
	if rr.Code != http.StatusOK || !strings.Contains(rr.Body.String(), "Ploos-AS GHCR stats") || !strings.Contains(rr.Body.String(), "/package/alpha") { t.Fatalf("dashboard: %d %s", rr.Code, rr.Body.String()) }

	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/package/alpha", nil))
	if rr.Code != http.StatusOK || !strings.Contains(rr.Body.String(), "alpha") || !strings.Contains(rr.Body.String(), "canvas") { t.Fatalf("package page: %d %s", rr.Code, rr.Body.String()) }
}
